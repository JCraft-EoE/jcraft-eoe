...
public class KQBTDRenderer extends GeoEntityRenderer<KQBTDEntity> {
    ...
    @Override
    public RenderLayer getRenderType(KQBTDEntity animatable, float partialTicks, MatrixStack stack,
    ...
    @Override
    public void render(GeoModel model, KQBTDEntity animatable, float partialTicks, RenderLayer type, MatrixStack matrixStackIn, VertexConsumerProvider renderTypeBuffer, VertexConsumer vertexBuilder, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
        float a = 1f;
        MinecraftClient mcClient = MinecraftClient.getInstance();
        if (mcClient.options.getPerspective().isFirstPerson() && mcClient.player != null)
