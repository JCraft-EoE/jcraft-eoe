...
public class GEREntity extends StandEntity implements IAnimatable, IAnimationTickable {
    ...
    public static Attack light = new Attack(2, 0.75f, 9, 5, 1.5, 5f, 0.75f, AttackType.BOX, 0.55f, -0.1f, 0, ModSoundRegister.IMPACT_1)
            .setInfo("Punch/Downward Kick", "quick combo starter, in air: more hitstun, less blockstun");
    public static Attack heavy = new Attack(17, 1f, 19, 10, 1.5, 9f, 1.1f, AttackType.BOX, 0.5f, 0, 0, ModSoundRegister.IMPACT_2).setHitspark(2).setArmor(true).setLaunch()
    ...
    public static Attack airlight = new Attack(2, 0.75f, 12, 5, 1.25, 4f, 0.75f, AttackType.BOX, 1, 0.33f, 0, ModSoundRegister.IMPACT_1)
            .setInfo("Downward Kick", "");
    public static Attack airheavy = new Attack(17, 1f, 24, 14, 1.5, 9f, 0.8f, AttackType.BOX, 2, 0.25f, 0, ModSoundRegister.IMPACT_1).setHitspark(2)
    ...
    private static HashMap<Entity, NbtCompound> rtzEntityData = new HashMap<>();

    private static final TrackedData<Integer> FLIGHTTIME;
    public Integer getFlightTime() { return this.dataTracker.get(FLIGHTTIME); }
    public void setFlightTime(int i) { this.dataTracker.set(FLIGHTTIME, i); }
    ...
    public void InitHeavyAttack() {
        ...
        if (data.user.isOnGround()) {
            if (HandleAttack(heavy, JCraft.standHeavyCD, 4))
                this.playSound(ModSoundRegister.GER_HEAVY, 1, 1);
        } else {
    ...
    public void InitBarrage() {
        ...
        if (data.user.isOnGround()) {
            ...
            if (HandleAttack(airbarrage, JCraft.standBarrageCD, 12))
                this.playSound(ModSoundRegister.GER_KICKBARRAGE, 1, 1);
        }
    }
    ...
    public void InitMiddleClick() {
        ...
        if (data.getInt(JCraft.standMMBCD) > 0) { return; }
    ...
    public void InitSpecial2() {
        ...
        if (HandleAttack(laser, JCraft.standS2CD, 8)) {
            this.playSound(ModSoundRegister.GER_LASER,1, 1);
        }
    }
    ...
    public void InitUlt() {
        ...
        if (rtzEntityData.isEmpty()) {
            ...
            if (HandleAttack(rtz, JCraft.standUltCD, 13)) {
                this.playSound(ModSoundRegister.GER_SETUP, 1, 1);
            }
        } else {
            // Fun part
    ...
    public void Desummon() {
        if (getFlightTime() > 0) { setFlightTime(0); return; }
        super.Desummon();
    }

    private static final Attack barrageFinisher = new Attack(17, 1f, 9, 6, 1.75, 1f, 1.1f, AttackType.BOX, 0.5f, 0, 0, ModSoundRegister.TW_KICK_HIT).setHitspark(2).setLaunch();
    @Override
    public void SpecialAttack(Attack attack, List<LivingEntity> entities) {
        ...
        if (attack == healself) {
            ...
            for (LivingEntity l : entities) {
                l.addStatusEffect(new StatusEffectInstance(ModStatusRegister.Knockdown, 30, 0, false, false));
            }
        } else if (attack == laser) {
            GERScorpionEntity scorpion = new GERScorpionEntity(ModEntityRegister.GERSCORPION, world);
    ...
    @Override
    public void tick() {
        if (age == 1) { this.world.playSound(null, this.getX(), this.getY(), this.getZ(), ModSoundRegister.GER_SUMMON, SoundCategory.PLAYERS, 1f, 1f); }
