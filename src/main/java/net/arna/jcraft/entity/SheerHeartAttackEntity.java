...
import net.arna.jcraft.entity.ai.goal.SHAAttackGoal;
...
public class SheerHeartAttackEntity extends MobEntity implements IAnimatable, IAnimationTickable {
    ...
    public void setOwner(LivingEntity owner) { this.owner = owner; this.setOwnerName(owner.getName().getString()); }
    ...
    public void tick() {
        ...
        if (!client) {
            if (this.owner == null) {
                ...
                if (this.age > 360 || !this.owner.isAlive()) { this.kill(); }
                ...
                if (target == null) {
                    if (this.age % 10 == 0) { // Entity lists are still expensive
                        ...
                        for (LivingEntity living : toTrack) {
                            if (!this.canTarget(living)) { continue; }
                            if (living.hasVehicle() && living.getVehicle() == this.owner) { continue; }
