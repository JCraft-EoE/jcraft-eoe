...
public class KnifeProjectile extends PersistentProjectileEntity implements IAnimatable {
    ...
    public KnifeProjectile(World world, LivingEntity owner) {
        super(ModEntityRegister.KNIFE, owner, world);
        this.setOwner(owner);
    }
    ...
    public Boolean getLightning() { return this.dataTracker.get(LIGHTNING); }
    public void setLightning(Boolean li) { this.dataTracker.set(LIGHTNING, li); }
                    ...
                    if (owner != null) {
                        if (delayTime < 1) {
                            if (!delayFired) {
                                ...
                                playSound(ModSoundRegister.TWOH_SHOOT, 1, 1);
