...
public abstract class StandEntity extends MobEntity {
    ...
    protected StandEntity(EntityType<? extends MobEntity> entityType, World world) { super(entityType, world); }
    ...
    public LivingEntity getUser() { return this.user; }
    ...
    public boolean getSameState() {return this.dataTracker.get(SAMESTATE); }
    public void setSameState(boolean samestate) { this.dataTracker.set(SAMESTATE, samestate); }

    public int getMoveStun() { return this.dataTracker.get(MOVESTUN); }
    public void setMoveStun(int moveStun) { this.dataTracker.set(MOVESTUN, moveStun); }

    public float getRotationOffset() { return this.dataTracker.get(ROTATIONOFFSET); }
    public void setRotationOffset(float rotationOffset) { this.dataTracker.set(ROTATIONOFFSET, rotationOffset); }

    public float getDistanceOffset() { return this.dataTracker.get(DISTANCEOFFSET); }
    public void setDistanceOffset(float distanceOffset) { this.dataTracker.set(DISTANCEOFFSET, distanceOffset); }

    public float getAlpha() { return this.dataTracker.get(ALPHA); }
    public void setAlpha(float alpha) { this.dataTracker.set(ALPHA, alpha); }

    public float getStandGauge() { return this.dataTracker.get(STANDGAUGE); }
    public void setStandGauge(float standGauge) { this.dataTracker.set(STANDGAUGE, standGauge); }

    public boolean getFree() { return this.dataTracker.get(FREE); }
    public void setFree(boolean free) { this.dataTracker.set(FREE, free); }
    ...
    public double getRemoteForwardInput() { return remoteForwardInput; }
    public double getRemoteSideInput() { return remoteSideInput; }
    public void setRemoteJumpInput(boolean b) { remoteJumpInput = b; }
    public boolean getRemoteJumpInput() { return remoteJumpInput; }
    ...
    public boolean getRemote() { return this.dataTracker.get(REMOTE); }
    ...
    public int getTSTime() { return this.dataTracker.get(TIMESTOPTIME); }
    public void setTSTime(int tsTime) { this.dataTracker.set(TIMESTOPTIME, tsTime); }

    @Override
    public SoundCategory getSoundCategory() { return SoundCategory.PLAYERS; }
    ...
    public boolean CanAttack() {
        if (hasUser()) {
            ...
            return this.getMoveStun() < 1 && timeStop.getTimeStopTicks() < 1 && !user.hasStatusEffect(ModStatusRegister.Dazed);
        }
        return false;
    }
    ...
    public CanAttackData CanAttackWithData() {
        if (hasUser()) {
            ITimeStop timeStop = (ITimeStop) user;
            return new CanAttackData(user, this.getMoveStun() < 1 && timeStop.getTimeStopTicks() < 1 && !user.hasStatusEffect(ModStatusRegister.Dazed));
        }
    ...
    public static void Stun(LivingEntity entity, int duration, int amplifier) {
        ...
        entity.addStatusEffect(new StatusEffectInstance(ModStatusRegister.Dazed, duration, amplifier, false, false, true));
    ...
    public void tick() {
        ...
        if (this.user == null) {
            if (world.isClient && this.getVehicle() instanceof LivingEntity living) { user = living; }
            return;
        ...
        if (client) {
            ...
            // Reset samestate
            if (this.getSameState()) { this.setSameState(false); }

            // Block break check
            if (this.getStandGauge() < 1) {
                user.addStatusEffect(new StatusEffectInstance(ModStatusRegister.Dazed, 40, 2));
            ...
            if (this.getMoveStun() >= 0 && !this.blocking && attack != null) {
                ...
                if (
                    ...
                    for (LivingEntity livingEntity : hurt) {
                        if (livingEntity instanceof StandEntity stand) {
                            ...
                            if (isBarrage && stand.curAttack != null && stand.curAttack.attackType == AttackType.BARRAGE) {
                                ...
                                this.world.playSound(null, midPos.x, midPos.y, midPos.z, ModSoundRegister.IMPACT_1, SoundCategory.NEUTRAL, 1, 0.5f);
                            }
                            continue;
                        }
                    ...
                    for (LivingEntity livingEntity : clashed) {
                        livingEntity.removeStatusEffect(ModStatusRegister.Dazed);
                        livingEntity.addStatusEffect(new StatusEffectInstance(ModStatusRegister.Dazed, 10, 3, true, false));
                    }
    ...
    public static void DamageLogic(World world, LivingEntity ent, Vec3d kbVec, int stunTicks, int stunType, boolean overrideStun, float damage, boolean lift, DamageSource playerSource, Entity attacker) {
        if (world.getGameRules().getBoolean(JCraft.COMBO_COUNTER) && !world.isClient()) {
            if (attacker instanceof PlayerEntity playerEntity) {
                ...
                if (comboCounter.getLastAttacked() != ent) { comboCounter.setComboCount(1); }
                else {
                    StatusEffectInstance stun = ent.getStatusEffect(ModStatusRegister.Dazed);
                    if (stun != null && stun.getAmplifier() == 1) {
                        //LOGGER.info("Target stun: " + stun.getDuration());
                        comboCounter.incrementComboCount();
                    } else { comboCounter.setComboCount(1); }
        ...
        if (ent.getFirstPassenger() instanceof StandEntity stand) {
            ...
            if (standAttack != null) {
                ...
                if (standAttack.attackType == AttackType.COUNTER && stand.getMoveStun() < (standAttack.moveStun - standAttack.initTime)) {
                    ...
                    ent.removeStatusEffect(ModStatusRegister.Dazed);
                    return;
                }
            ...
            if (stand.blocking && !stand.getRemote()) {
                ...
                stand.playSound(ModSoundRegister.STAND_BLOCK, 1, 1);
                hit = false;
            }
        }
    ...
    public void stopRiding() {
        if (!getRemote()) {
            this.playSound(ModSoundRegister.STAND_DESUMMON, 1, 1);
            this.kill();
        }
        super.stopRiding();
    }
    ...
    public void MobAI(MobEntity mob, LivingEntity target) {
        ...
        boolean stunned = mob.hasStatusEffect(ModStatusRegister.Dazed);
        // If stunned, and about to get hit by another move, combo break sometimes
        if (stunned) {
            StatusEffectInstance mobStun = mob.getStatusEffect(ModStatusRegister.Dazed);
        ...
        if (!this.blocking) {
            StatusEffectInstance stun = target.getStatusEffect(ModStatusRegister.Dazed);
            ...
            if (stunTicks > 0) {
                float back = -0.5f;
                if (enemyHasStand && enemyStand.blocking) { back = 0f; }
