...
public class KQBTDEntity extends KillerQueenEntity implements IAnimatable, IAnimationTickable {
    ...
    public static Attack light = new Attack(2, 0.75f, 19, 0, 1.5, 3.5f, 0.75f, AttackType.MULTIHIT, 1f, 0, List.of(6, 11), ModSoundRegister.IMPACT_4)
            .setInfo("Dual Punch", "quick combo starter");
    public static Attack heavy = new Attack(12, 0.75f, 9, 5, 1, 7.5f, 1.1f, AttackType.BOX, 0.5f, 0, 0, ModSoundRegister.IMPACT_4).setHitspark(2).setLaunch()
            .setInfo("Elbow", "fast, short-range knockback");
    public static Attack barrage = new Attack(17, 0.75f, 50, 0, 1.5, 1f, 0.1f, AttackType.BARRAGE, 1, 0, 3, ModSoundRegister.IMPACT_4)
    ...
    public static Attack detonate = new Attack(1, 0.75f, 6, 5, 0, 0f, 0.0f, AttackType.BOX)
            .setInfo("Detonate", "crouch with a bomb planted within 20s on a living being to activate Bites the Dust");
    public static Attack bubble = new Attack(23, 0.75f, 18, 15, 0, 0f, 0.0f, AttackType.BOX).setRanged(true)
    ...
    public void InitHeavyAttack() {
        ...
        if (HandleAttack(heavy, JCraft.standHeavyCD, 4)) {
            this.playSound(ModSoundRegister.KQBTD_ELBOW,1, 1);
        }
    }
    ...
    public void InitBarrage() {
        ...
        if (HandleAttack(barrage, JCraft.standBarrageCD, 5)) {
            this.playSound(ModSoundRegister.KQ_BARRAGE,1, 1);
        }
    }
    ...
    public void InitUlt() {
        ...
        if (HandleAttack(detonate, JCraft.standUltCD, 6)) {
            this.playSound(ModSoundRegister.KQ_DETONATE,1, 1);
        }
    }
    ...
    public void InitSpecial2() {
        ...
        if (HandleAttack(bubble, JCraft.standS2CD, 8)) {
            this.playSound(ModSoundRegister.KQ_UPPERCUT,1, 1);
        }
    }
    ...
    public void InitSpecial3() {
        ...
        this.coin = new ItemEntity(world, user.getX(), user.getY() + user.getHeight() * 2 / 3, user.getZ(), new ItemStack(ModItemRegister.KQCOIN, 1), lookVec.x, lookVec.y, lookVec.z);
        ...
        if (attack == bombplant) {
            ...
            if (bombEntity instanceof LivingEntity livingEntity) {
                if (user.isSneaking()) {
                    ...
                    livingEntity.addStatusEffect(new StatusEffectInstance(ModStatusRegister.Knockdown, 35, 0, true, false));
                }
            } else {
                Vec3d bombPos = null;
    ...
    public MoveSelectionResult SpecificMoveSelectionCriterion(Attack attack, MobEntity mob, LivingEntity target, int stunTicks, int enemyMoveStun, double distance, StandEntity enemyStand, Attack enemyAttack) {
        ...
        if (bombPos != null && attack == detonate && target.squaredDistanceTo(bombPos) < 9.0D) { return MoveSelectionResult.USE; }
        return MoveSelectionResult.PASS;
    }

    @Override
    public void tick() {
        if (age == 1) { this.world.playSound(null, this.getX(), this.getY(), this.getZ(), ModSoundRegister.KQBTD_SUMMON, SoundCategory.PLAYERS, 1f, 1f); }
            ...
            if (world.isClient) {
                ...
                if (user instanceof PlayerEntity playerEntity) {
                    ...
                    if (bombExists) {
                        ...
                        for (LivingEntity l:
                                list) {
                            if (l.squaredDistanceTo(bPos) < 9) { anyInRange = true; break; }
                        }
