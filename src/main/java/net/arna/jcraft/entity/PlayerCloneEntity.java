...
import net.arna.jcraft.entity.ai.goal.CloneAttackGoal;
...
public class PlayerCloneEntity extends HostileEntity implements RangedAttackMob {
    ...
    public String getOwnerName() { return this.dataTracker.get(OWNERNAME); }
    public void setOwnerName(String state) { this.dataTracker.set(OWNERNAME, state); }
    ...
    public void tick() {
        ...
        if (client) {
            if (this.age == 1 && this.getType() == ModEntityRegister.PLAYERCLONE) {
            ...
            if (this.switched && this.switchedTo.age > 10) { this.discard(); } // Remove outdated clones
            ...
            if (this.owner == null) {
                ...
                if (attacker != null) { this.setTarget(attacker); }
            } else {
                if (this.persistTarget == null) {
                    LivingEntity attacking = owner.getAttacking();
                    if (attacking != null && attacking.isAlive()) { this.persistTarget = attacking; }
                    ...
                    this.setTarget(null);
                    if (!this.getNavigation().isIdle()) { this.getNavigation().stop(); }
                }
            }
        }
    }
