...
import net.arna.jcraft.util.AttackQueue;
import net.arna.jcraft.util.AttackType;
...
public class GoldenExperienceEntity extends StandEntity implements IAnimatable, IAnimationTickable {
    ...
    public static Attack light = new Attack(2, 0.75f, 9, 6, 1.5, 5f, 0.75f, AttackType.BOX, 0.5f, -0.1f, 0, ModSoundRegister.IMPACT_1)
            .setInfo("Punch", "quick combo starter");
    public static Attack heavy = new Attack(17, 1f, 22, 13, 1.5, 9f, 1.5f, AttackType.BOX, 0.5f, 0, 0, ModSoundRegister.IMPACT_2).setHitspark(2).setArmor(true).setLaunch()
    ...
    public static Attack tree = new Attack(20, 1f, 24, 14, 1.25, 5f, 0.2f, AttackType.BOX, 0.75f, -0.1f, 0, ModSoundRegister.IMPACT_2).setHitspark(2)
    ...
    public static Attack overclock = new Attack(46, 1f, 31, 22, 2, 9f, 0.9f, AttackType.BOX, 3, 0, 0, ModSoundRegister.IMPACT_5).setHitspark(2)
            .setUB(false)
            .setInfo("Overclock", "slow, unblockable, devastating stun");
    public static Attack rekka3 = new Attack(23, 1f, 24, 12, 2, 7f, 0.5f, AttackType.BOX, 0.75f, 0, 0, ModSoundRegister.TW_KICK_HIT).setHitspark(2);
    public static Attack rekka2 = new Attack(23, 1f, 18, 10, 1.75, 5f, 0.5f, AttackType.BOX, 0.75f, 0, 0, ModSoundRegister.IMPACT_2)
            .setInfo("how did you", "get here", AttackQueue.SPECIAL2).setHitspark(2).setFollowup(rekka3);
    public static Attack rekka1 = new Attack(23, 1f, 20, 8, 1.5, 5f, 0.5f, AttackType.BOX, 0.75f, 0, 0, ModSoundRegister.IMPACT_2)
    ...
    public void InitBarrage() {
        ...
        if (HandleAttack(barrage, JCraft.standBarrageCD, 5)) {
            this.playSound(ModSoundRegister.GE_BARRAGE,1, 1);
        }
    }
    ...
    public void InitSpecial1() {
        ...
        if (data.user.isSneaking()) {
            if (HandleAttack(heal, JCraft.standS1CD, 7)) {
                this.playSound(ModSoundRegister.GE_HEAL, 1, 1);
            }
    ...
    public void InitSpecial2() {
        if (hasUser()) {
            ...
            if (user.hasStatusEffect(ModStatusRegister.Dazed)) { return; }
    ...
    public void InitMiddleClick() {
        ...
        if (HandleAttack(tree, JCraft.standMMBCD, 8)) {
            this.playSound(ModSoundRegister.GE_TREE,1, 1);
        }
    }
    ...
    public void SpecialAttack(Attack attack, List<LivingEntity> entities) {
        if (hasUser()) {
            ...
            if (attack == healself) {
                ...
                GETreeEntity tree = new GETreeEntity(ModEntityRegister.GETREE, world);
                tree.owner = user;
                ...
                GESnakeEntity snake = new GESnakeEntity(ModEntityRegister.GESNAKE, world);
                ...
                for (LivingEntity ent:
                    ...
                    if (!JCraftUtils.isBlocking(ent)) {
                        ent.addStatusEffect(new StatusEffectInstance(ModStatusRegister.Knockdown, 50, 0, true, false));
                    }
                }
                ...
                for (LivingEntity ent:
                    ...
                    ent.addStatusEffect(new StatusEffectInstance(ModStatusRegister.Dazed, 60, 1, true, false));
                    ent.addStatusEffect(new StatusEffectInstance(ModStatusRegister.OutOfBody, 60, 0, false, true));
                }
            }
        }
    }
    ...
    public MoveSelectionResult SpecificMoveSelectionCriterion(Attack attack, MobEntity mob, LivingEntity target, int stunTicks, int enemyMoveStun, double distance, StandEntity enemyStand, Attack enemyAttack) {
        if (attack == snake) {
            if (mob.getMainHandStack().isEmpty() && mob.getOffHandStack().isEmpty()) { return MoveSelectionResult.STOP; }
    ...
    @Override
    public void tick() {
        if (age == 1) { this.world.playSound(null, this.getX(), this.getY(), this.getZ(), ModSoundRegister.GE_SUMMON, SoundCategory.PLAYERS, 1f, 1f); }
