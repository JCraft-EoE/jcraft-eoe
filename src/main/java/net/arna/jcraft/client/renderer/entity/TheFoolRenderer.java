...
public class TheFoolRenderer extends GeoEntityRenderer<TheFoolEntity> {
    ...
    @Override
    public RenderLayer getRenderType(TheFoolEntity animatable, float partialTicks, MatrixStack stack,
    ...
    /*
    @Override
    public void renderEarly(TheFoolEntity animatable, MatrixStack poseStack, float partialTick, VertexConsumerProvider bufferSource, VertexConsumer buffer, int packedLight, int packedOverlay, float red, float green, float blue, float partialTicks) {
    ...
    @Override
    public void render(GeoModel model, TheFoolEntity animatable, float partialTicks, RenderLayer type, MatrixStack matrixStackIn, VertexConsumerProvider renderTypeBuffer, VertexConsumer vertexBuilder, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
        float a = 1f;
        MinecraftClient mcClient = MinecraftClient.getInstance();
        if (mcClient.options.getPerspective().isFirstPerson() && mcClient.player != null) {
