...
import net.arna.jcraft.entity.GoldenExperienceEntity;
...
public class GoldenExperienceRenderer extends GeoEntityRenderer<GoldenExperienceEntity> {
    ...
    @Override
    public RenderLayer getRenderType(GoldenExperienceEntity animatable, float partialTicks, MatrixStack stack,
    ...
    @Override
    public void render(GeoModel model, GoldenExperienceEntity animatable, float partialTicks, RenderLayer type, MatrixStack matrixStackIn, VertexConsumerProvider renderTypeBuffer, VertexConsumer vertexBuilder, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha) {
        float a = 1f;
        MinecraftClient mcClient = MinecraftClient.getInstance();
        if (mcClient.options.getPerspective().isFirstPerson() && mcClient.player != null) {
