...
public class BTDDetonateAttack extends AbstractMove<BTDDetonateAttack, AbstractKillerQueenEntity<?, ?>> {
    ...
    public @NotNull Set<LivingEntity> perform(AbstractKillerQueenEntity<?, ?> stand, LivingEntity user, MoveContext ctx) {
        ...
        JCraft.createParticle((ServerWorld) stand.world, pos.x, pos.y + 2, pos.z, JParticleType.BITES_THE_DUST);
        ...
        List<LivingEntity> list = stand.world.getEntitiesByClass(LivingEntity.class, new Box(v1, v2),
                EntityPredicates.VALID_LIVING_ENTITY.and(e -> e != user.getVehicle() && e != user && e != stand && e != btdEntity));

        for (LivingEntity l : list)
            if (l.squaredDistanceTo(pos) < 9) {
                if (l.squaredDistanceTo(pos) < 2.25)
                    stand.world.createExplosion(user, l.getX(), l.getY() + l.getHeight() / 2, l.getZ(), 1.5f, Explosion.DestructionType.NONE);
                else stand.world.createExplosion(user, l.getX(), l.getY() + l.getHeight() / 2, l.getZ(), 1f, Explosion.DestructionType.NONE);
            }
