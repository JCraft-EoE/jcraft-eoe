...
public class BTDDetonateAttack extends AbstractMove<BTDDetonateAttack, AbstractKillerQueenEntity<?, ?>> {
    ...
    public @NonNull Set<LivingEntity> perform(AbstractKillerQueenEntity<?, ?> attacker, LivingEntity user, MoveContext ctx) {
        ...
        attacker.world.createExplosion(user, btdEntity.getX(), btdEntity.getY() + btdEntity.getHeight() / 2, btdEntity.getZ(), 2f, Explosion.DestructionType.NONE);
        ...
        for (LivingEntity l : list)
            if (l.squaredDistanceTo(pos) < 9) {
                if (l.squaredDistanceTo(pos) < 2.25)
                    attacker.world.createExplosion(user, l.getX(), l.getY() + l.getHeight() / 2, l.getZ(), 1.5f, Explosion.DestructionType.NONE);
                else attacker.world.createExplosion(user, l.getX(), l.getY() + l.getHeight() / 2, l.getZ(), 1f, Explosion.DestructionType.NONE);
            }
