    ...
    public static void serverTick(MinecraftServer server) {
        ...
        for (ServerWorld serverWorld : server.getWorlds()) {
            ...
            for (MobEntity mob : mobEntities) {
                ...
                if (!mob.isAlive() || mobData == null) continue;

                // Damage timer
                if (mob.getAttacker() != null) mobData.putInt("DamageTimer", 600);
                ...
                } else if (mobData.contains("StandID")) JCraft.summon(serverWorld, mob);

                // Handle cooldowns
                for (String cooldownType : JCraft.cooldowns) {
                    if (!mobData.contains(cooldownType)) mobData.putInt(cooldownType, 0);
