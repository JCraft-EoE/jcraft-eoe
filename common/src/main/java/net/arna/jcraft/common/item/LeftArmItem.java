...
/**
 * Left Arm corpse part item that grants Tusk Act 1.
 * Uses a two-step confirmation system similar to StandArrowItem.
 */
public class LeftArmItem extends Item {
    protected Set<Player> warned = Collections.newSetFromMap(new WeakHashMap<>());

    public LeftArmItem(Properties settings) {
        super(settings);
    }
    ...
    public @NonNull InteractionResultHolder<ItemStack> use(@NonNull final Level world, final @NonNull Player user,
        ...
        if (!world.isClientSide()) {
            ...
            // Grant Tusk Act 1
            standData.setType(JStandTypeRegistry.TUSK_ACT_1.get());
